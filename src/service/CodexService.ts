...
import { StreamReplyResponse, convertGeminiHistoryToGPT, getSystemPrompt } from '@/util/ChatConfigUtil.ts';
...
/**
 * Generates a follow-up response for Codex after executing tools
 * @param messages - Chat messages in OpenAI chat format
 * @param model - Model name
 * @returns Reader for the follow-up SSE stream mapped to OpenAI chunk format
 */
async function generateFollowup(messages: any[], model: string): Promise<ReadableStreamDefaultReader<Uint8Array>> {
  const modified = ToolUsageAdapter.modifyMessagesWithToolInfo(messages);
  const payload = {
    model,
    instructions: await getCodexInstructions(),
    input: modified,
  ...
  if (!resp.ok || !resp.body) {
    ...
    throw new Error(`Failed to generate followup: ${resp.status} ${resp.statusText} ${errorBody}`);
  }
