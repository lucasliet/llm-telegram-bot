  ...
  while (true) {
    ...
    if (result.length > 4093) {
      ...
      if (result.length > 4093) {
        ...
        lastUpdate = await editMessageWithCompletionEvery3Seconds(this, message_id, result, lastUpdate, true);
        onComplete(result);
        return this.streamReply(reader, onComplete, responseMap, remainingChunk);
      }
    }
    
    // Update message periodically
    lastUpdate = await editMessageWithCompletionEvery3Seconds(this, message_id, result, lastUpdate);
  }
 ...
 */
function decodeStreamResponseText(responseMessage: Uint8Array, responseMap?: (responseBody: string) => string): string {
...
async function editMessageWithCompletionEvery3Seconds(
  ...
  if (isLastMessage || has2SecondsPassed) {
    ...
    try {
      await ctx.api.editMessageText(ctx.chat!.id, messageId, displayMessage, { parse_mode: 'Markdown' });
    } catch (error) {
