...
export default {
  async generateText(userKey: string, quote: string = '', prompt: string, model: string = 'claude-3-5-sonnet-latest'): Promise<StreamReplyResponse> {
    ...
    const apiResponse = await fetch('https://api.puter.com/drivers/call', {
      ...
      body: JSON.stringify({
        ...
        args: {
          messages: [
            { role: "system", content: replaceGeminiConfigFromTone('Claude', model, 8000) },
