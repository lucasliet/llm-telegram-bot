 ...
 */
export async function handleCloudflare(ctx: Context, commandMessage?: string): Promise<void> {
  ...
  if (cloudflareCommand === 'llama') {
    const { reader, onComplete, responseMap } = await CloudFlareService.generateText(
      userKey, 
      quote, 
    ...
    const { reader, onComplete, responseMap } = await CloudFlareService.generateSQL(
      userKey, 
      quote, 
      message!.replace('sql:', '')
    );
    ...
    const { reader, onComplete, responseMap } = await CloudFlareService.generateCode(
      userKey, 
      quote, 
    ...
    ctx.replyWithPhoto(
      new InputFile(new Uint8Array(await CloudFlareService.generateImage(message!)), 'image/png'), 
