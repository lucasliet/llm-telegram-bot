 ...
 */
export async function handleOpenAI(ctx: Context, commandMessage?: string): Promise<void> {
  ...
  if (photos && caption) {
    ...
    const { reader, onComplete, responseMap } = await openAIService.generateTextFromImage(
      userKey, 
      quote, 
      photosUrl, 
      caption
  ...
  if (command === 'gpt') {
    const { reader, onComplete, responseMap } = await new OpenAiService('/github').generateText(
      userKey, 
      quote, 
      message!.replace('gpt:', '')
    );
