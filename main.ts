...
import { adminKeyboard, userKeyboard, adminHelpMessage, userHelpMessage } from './config/KeyboardConfig.ts';
...
function configureMiddleware() {
  APP.use(async (ctx, next) => {
    try {
      ...
      ctx.response.body = { message: err instanceof Error ? err.message : 'Unknown error occurred' };
    }
  });
}
