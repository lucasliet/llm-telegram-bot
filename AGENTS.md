...
- **IMPORTANTE:** NÃO rode `deno fmt` sem argumentos. O formatter do Deno reformata arquivos `.md`, `.json` e `.yml` de forma indesejada. Use sempre globs para
  formatar apenas código-fonte TypeScript.
...
- **Never read env var values into context** - only check if they are filled. Use `$VAR` directly in bash commands or `Deno.env.get("VAR")` in code to avoid
  exposing secrets in AI context. Example: `echo ${VAR:-"vazia"}` shows only status, not the actual value.
