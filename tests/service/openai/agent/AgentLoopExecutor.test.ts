...
import { StreamProcessor, ExtractedToolCall, StreamProcessingResult } from '@/service/openai/stream/StreamProcessor.ts';
import { ToolExecutionResult, AgentLoopConfig } from '@/service/openai/agent/AgentLoopConfig.ts';
...
Deno.test({ name: 'Fluxo básico - shouldCompleteWithoutToolCalls', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Fluxo básico - shouldExecuteSingleToolAndComplete', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Fluxo básico - shouldExecuteMultipleToolsSequentially', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Fluxo básico - shouldHandleMultipleIterations', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Limite de iterações - shouldStopAtMaxIterations', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Limite de iterações - shouldRespectCustomMaxIterations', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Execução de tools - shouldHandleToolNotFound', ...testOpts, fn: async () => {
	const toolCalls: ExtractedToolCall[][] = [
...
Deno.test({ name: 'Execução de tools - shouldHandleToolExecutionError', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Execução de tools - shouldTimeoutLongRunningTools', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Execução de tools - shouldPassArgumentsCorrectly', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Sumarização - shouldNotSummarizeWhenUnderLimit', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Sumarização - shouldSummarizeWhenOverLimit', ...testOpts, fn: async () => {
	let summarizationCalled = false;
...
Deno.test({ name: 'Sumarização - shouldIncludeResponseBuffer (Bug #2 fix validation)', ...testOpts, fn: async () => {
	let summarizationCalled = false;
...
Deno.test({ name: 'Sumarização - shouldTruncateOnSummarizationFailure', ...testOpts, fn: async () => {
	const largeResult = generateLargeString(50000);
...
Deno.test({ name: 'Erros - shouldEmitErrorOnStreamFailure', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Erros - shouldContinueAfterToolError', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Callbacks - shouldCallOnIterationStart', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Callbacks - shouldCallOnToolExecution', ...testOpts, fn: async () => {
...
Deno.test({ name: 'Callbacks - shouldCallOnIterationComplete', ...testOpts, fn: async () => {
