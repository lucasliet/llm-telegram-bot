...
Deno.test("ToolService.schemas exposes tools list", () => {
  const ok =
    Array.isArray(ToolService.schemas) && ToolService.schemas.length > 0;
...
Deno.test("ToolService search_searx returns mapped results", async () => {
  ...
  try {
    ...
    const fn = (ToolService as any).tools.get("search_searx").fn as (args: {
      query: string;
      num_results: number;
    }) => Promise<any[]>;
...
Deno.test(
  "ToolService fetch returns plain text and parses HTML body",
  async () => {
    const html =
      "<html><body><div>Hello</div><script>bad()</script></body></html>";
    const original = globalThis.fetch;
    try {
      globalThis.fetch = mockFetchSequence([
        new Response(html, {
      ...
      const fn = (ToolService as any).tools.get("fetch").fn as (args: {
        url: string;
      }) => Promise<string>;
    ...
    const innertube = {
      ...
      captions: {
        playerCaptionsTracklistRenderer: {
          captionTracks: [{ languageCode: "en", baseUrl: "https://c" }],
          translationLanguages: [{ languageCode: "pt" }],
        },
      },
    };
    const xml =
      '<transcript><text start="0" dur="1.0">Hello</text></transcript>';
    const original = globalThis.fetch;
    try {
      globalThis.fetch = mockFetchSequence([
      ...
      const fn = (ToolService as any).tools.get("transcript_yt").fn as (args: {
        videoUrl: string;
...
Deno.test(
  "ToolService transcript_yt returns null for invalid URL",
  async () => {
    const fn = (ToolService as any).tools.get("transcript_yt").fn as (args: {
      videoUrl: string;
      preferredLanguages?: string[];
...
Deno.test("ToolService transcript_yt accepts live URL format", async () => {
  ...
  try {
    ...
    const segments = await fn({
      videoUrl: "https://www.youtube.com/live/aaaaaaaaaaa",
    });
    assertEquals(Array.isArray(segments) && segments.length > 0, true);
  } finally {
    globalThis.fetch = original;
...
Deno.test("ToolService copilot_usage returns JSON on success", async () => {
  ...
  try {
    Deno.env.get = (k: string) =>
      (k === "COPILOT_GITHUB_TOKEN" ? "tok" : originalGet(k)) as any;
    globalThis.fetch = mockFetchSequence([
