...
Deno.test('GithubCopilotHandler forwards to service with chosen model', async () => {
	const originalOpenKv = Deno.openKv;
	mockDenoEnv({ OPENAI_API_KEY: 'x', COPILOT_TOKEN: 'z' });
	Deno.openKv = () =>
		Promise.resolve(
			{
	...
	try {
		...
		const svc = await import('../../src/service/openai/GithubCopilotService.ts');
