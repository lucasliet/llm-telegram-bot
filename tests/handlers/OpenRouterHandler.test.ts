...
Deno.test('OpenRouterHandler streams text path', async () => {
	mockDenoEnv({ OPENROUTER_API_KEY: 'x' });
	const restore = setupKvStub();
	const ctx: any = {
		streamReply: spy(() => Promise.resolve()),
		extractContextKeys: spy(() => Promise.resolve({ userKey: 'user:1', contextMessage: 'llama: hi', photos: undefined, caption: undefined, quote: undefined })),
	};
	await import('../../src/service/TelegramService.ts');
	const mod = await import('../../src/handlers/OpenRouterHandler.ts');
	const svc = await import('../../src/service/openai/OpenrouterService.ts');
	...
	await mod.handleOpenRouter(ctx);
