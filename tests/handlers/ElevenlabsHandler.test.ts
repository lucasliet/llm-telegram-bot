...
Deno.test('ElevenlabsHandler extracts context correctly for fala command', async () => {
	...
	// This will call textToSpeech which will fail, but we can catch and verify the logic was correct
	try {
		await mod.handleFala(ctx, textModelService as any);
	} catch (_error) {
		// Expected to fail due to API call, but we can still verify the logic
	}
